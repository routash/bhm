			...
			<div class="page-content">
				...
				<div class="row">
					<div class="col-xl-10 mx-auto">
						<h4> <marquee class="mb-0 text-uppercase">LEARNING LICENCE TEST PASS 10 MINT ME.</marquee></h4>
						<hr/>
						...
						<div class="card border-top border-0 border-4 border-primary">
							<div class="card-body p-5">
								<div class="card-title d-flex align-items-center">
									...
									<h5 class="mb-0 text-primary">Enter Learning Licence Details</h5>
								</div>
								<hr>
								<form action="" method="POST" class="row g-3">
									<div class="col-md-3">
										<label for="inputFirstName" class="form-label">Application Number</label>
										<input name="application_no" type="text"  placeholder="Name As Per Aadhaar"  class="form-control" id="inputFirstName">
									</div>
									...
									<div id="maineid" class="col-md-3">
										...
										<input name="password" type="text"    placeholder="Enter Your Password" class="form-control" >
									</div>
									...
									<h5 class="text-warning ">Application Fee: <?php  
										$price = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT price FROM pricing WHERE service_name='lltext'")); 
										echo "₹" .$price['price'];
										?></h5>
                    ...
                    <table id="example2" class="table align-middle mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="text-center">Remarak</th>
...
$res = mysqli_query($ahk_conn,"SELECT * FROM lltext WHERE appliedby='".$udata['phone']."'  ORDER BY id DESC");
if(mysqli_num_rows($res)>0){
    ...
    while($data = mysqli_fetch_assoc($res)){
        ...
        <tr>
            ...
            <td class="text-center"><?php echo strtoupper($data['password']); ?></td>
            <td class="text-center"><?php echo strtoupper($data['state']); ?></td>
            <td class="text-center"><?php
            if(!$data['massage'] ==NULL){
                ?>
                <?php echo strtoupper($data['massage']); ?>
                <?php
            }else{
                echo "L.L Text Not generated Yet.";
            }
            ?></td>
                    ...
                    if($data['status']=="pending"){
                        ...
                        <?php
                         ...
                         <div class="text-center">
                                <a download="<?php echo $data['massage'] ?>" href="<?php echo $data['pdf_link'] ?>" class="btn btn-sm btn-success">Download PDF</a>
                         </div>
                            <?php
