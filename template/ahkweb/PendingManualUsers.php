<?php 
    ...
    if(checkAdmin($udata['type'] == true)){
        if(isset($_GET['apid']) && $_GET['approve'] == 1 ){
           ...
           $update = mysqli_query($ahk_conn,"UPDATE users SET status='1' WHERE order_id='$apid'");
           if($update && $pupdate){
               ?>
                <script>
                   $(function(){
               ...
               <?php
               ahkRedirect('PendingManualUsers.php',1000);
           }else{
               ?>
                <script>
                    $(function(){
        ...
        <div class="card radius-10">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    <div>
                        <h5 class="mb-0">All manual QR Paid Users</h5>
                    </div>
...
INNER JOIN users
ON wallet.txn_id = users.order_id
WHERE wallet.PAYMENTMODE= 'REGQR' AND wallet.status = 'pending' 
        ...
        <tr>
            ...
            <td  class="text-center">
                <div class="mr-12 font-24 ">
                    ...
                    <a class="text-success bg-light-success" title="Approve User" href="?apid=<?php echo base64_encode($data['txn_id']); ?>&approve=1"><i class='bx bx-check'></i></a>
                    <a class="text-danger bg-light-danger" title="Reject User" href="?rejid=<?php echo base64_encode($data['txn_id']); ?>&reject=1"><i class='bx bx-x'></i></a>
                </div>
