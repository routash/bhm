...
include('../template/ahkweb/biometric_issues_admin_list.php');
if(isset($_POST['status']) && !empty($_POST['status']) && $_POST['status']  && $_POST['aadhaar_no']){
 ...
 $id = mysqli_real_escape_string($ahk_conn,$_POST['id']);
    if($status == 'refunded'){
        $sl = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM biometric WHERE id='$id' LIMIT 1"));
        $amount = $sl['fee'];
        $updt = mysqli_query($ahk_conn,"UPDATE users SET balance=balance+$amount WHERE phone='".$sl['appliedby']."'");
        $sql = mysqli_query($ahk_conn,"UPDATE biometric SET  status='$status', aadhaar_no='$aadhaar_no' WHERE id='$id'");
        if($updt && $sql){
            showAlert('Refund Success');
            ahkRedirect('biometric_issues_admin_list.php',1200);
        }
