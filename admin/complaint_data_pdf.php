...
include('../template/ahkweb/complaint_data_pdf.php');
if(isset($_POST['eid']) && !empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['time']) ){
    ...
    $price = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM pricing WHERE service_name='aadhaarpdf' "));
    if($udata['balance']>=$price['price']){
        ...
        $submit = mysqli_query($ahk_conn,"INSERT INTO `aadhaarpdf` (`appliedby`, `name`, `eid`, `date`, `time`, `status`, `fee`) VALUES('$appliedby','$name','$eid','$date','$time','pending','$fee')");
        if($submit){
            ?>
            <script>
                $(function(){
