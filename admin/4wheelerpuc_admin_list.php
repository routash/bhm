...
include('../template/ahkweb/4wheelerpuc_admin_list.php');
if(isset($_POST['status']) && !empty($_POST['status']) && $_POST['status']  && $_POST['remark']){
 ...
 $id = mysqli_real_escape_string($ahk_conn,$_POST['id']);
   ...
   if($status == 'refunded'){
        $sl = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM 4wheelerpuc WHERE id='$id' LIMIT 1"));
        $amount = $sl['fee'];
        $updt = mysqli_query($ahk_conn,"UPDATE users SET balance=balance+$amount WHERE phone='".$sl['appliedby']."'");
        $sql = mysqli_query($ahk_conn,"UPDATE 4wheelerpuc SET  pdf_link='$link', status='$status', remark='$remark' WHERE id='$id'");
        if($updt && $sql){
            showAlert('Refund Success');
            ahkRedirect('4wheelerpuc_admin_list.php',1200);
        }
