...
include('../template/ahkweb/rcdetsils_admin_list.php');
if(isset($_POST['status']) && !empty($_POST['status']) && $_POST['status']  && $_POST['massage']){
 $status = mysqli_real_escape_string($ahk_conn,$_POST['status']);
 $massage = mysqli_real_escape_string($ahk_conn,$_POST['massage']);
 $id = mysqli_real_escape_string($ahk_conn,$_POST['id']);
   ...
   if($status == 'refunded'){
        $sl = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM rc_print WHERE id='$id' LIMIT 1"));
        $amount = $sl['fee'];
        $updt = mysqli_query($ahk_conn,"UPDATE users SET balance=balance+$amount WHERE phone='".$sl['appliedby']."'");
        $sql = mysqli_query($ahk_conn,"UPDATE rc_print SET  pdf_link='$link', status='$status', massage='$massage' WHERE id='$id'");
        if($updt && $sql){
            showAlert('Refund Success');
            ahkRedirect('rcdetsils_admin_list.php',1200);
        }
    }else if($status == 'success'){
    $sql = mysqli_query($ahk_conn,"UPDATE rc_print SET  pdf_link='$link', status='$status', massage='$massage' WHERE id='$id'");
    if($sql){
            showAlert('RC PDF Updated Successfully!');
            ahkRedirect('rcdetsils_admin_list.php',1200);
    }
   }else{
    ?>
