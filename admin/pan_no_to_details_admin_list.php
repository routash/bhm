...
include('../template/ahkweb/pan_no_to_details_admin_list.php');
if(isset($_POST['status']) && !empty($_POST['status']) && $_POST['status']  && $_POST['remark']){
 ...
 $id = mysqli_real_escape_string($ahk_conn,$_POST['id']);
   ...
   if($status == 'refunded'){
        $sl = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM pan_details WHERE id='$id' LIMIT 1"));
        $amount = $sl['fee'];
        $updt = mysqli_query($ahk_conn,"UPDATE users SET balance=balance+$amount WHERE phone='".$sl['appliedby']."'");
        $sql = mysqli_query($ahk_conn,"UPDATE pan_details SET  details_link='$link', status='$status', remark='$remark' WHERE id='$id'");
        if($updt && $sql){
            showAlert('Refund Success');
            ahkRedirect('pan_no_to_details_admin_list.php',1200);
        }
    }else if($status == 'success'){
    $sql = mysqli_query($ahk_conn,"UPDATE pan_details SET  details_link='$link', status='$status', remark='$remark' WHERE id='$id'");
    if($sql){
       showAlert('Pan Details Updated Successfully!');
       ahkRedirect('pan_no_to_details_admin_list.php',1200);
    }
   }else{
    ?>
