...
include('../template/ahkweb/biometric_issues.php');
if(isset($_POST['eid']) && !empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['time']) ){
    ...
    $price = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM pricing WHERE service_name='biometric' "));
    if($udata['balance']>=$price['price']){
        ...
        $submit = mysqli_query($ahk_conn,"INSERT INTO `aadhaar_no` (`appliedby`, `name`, `eid`, `date`, `time`, `status`, `fee`,`purpose`) VALUES('$appliedby','$name','$eid','$date','$time','pending','$fee','Biometric')");
       if($submit){
            ?>
            <script>
                $(function(){
