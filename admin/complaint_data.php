...
include('../template/ahkweb/complaint_data.php');
if(isset($_POST['eid']) && !empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['time']) ){
    ...
    $price = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM pricing WHERE service_name='complaint_data' "));
    if($udata['balance']>=$price['price']){
        ...
        $updatehistory = mysqli_query($ahk_conn,"INSERT INTO `wallethistory`(`userid`, `amount`, `balance`, `purpose`, `status`, `type`) VALUES ('$appliedby','$fee','$nbal','Complaint PDF Find','1','Debit')");
        if($debit && $updatehistory){
        $submit = mysqli_query($ahk_conn,"INSERT INTO `aadhaar_no` (`appliedby`, `name`, `eid`, `date`, `time`, `status`, `fee`,`purpose`) VALUES('$appliedby','$name','$eid','$date','$time','pending','$fee','Complaint')");
        if($submit){
            ?>
            <script>
                $(function(){
